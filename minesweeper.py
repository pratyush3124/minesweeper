...
class Game(Tk):
    ...
    def settings(self):
        ...
        a = Button(self.settingsw, text = '7x7', command = lambda: self.configure(5,5,5))
        a.pack()

        b = Button(self.settingsw, text = '10x10', command = lambda: self.configure(10,10,10))
        b.pack()

        c = Button(self.settingsw, text = '15x15', command = lambda: self.configure(15,15,20))
        c.pack()

        custom = Button(self.settingsw, text = 'Custom', command=lambda:self.customscreen())
        custom.pack()

    def configure(self,r,c,n):
        self.destroy()
    ...
    def customscreen(self):
        ...
        button1 = Button(self.csw,text='START',bg='brown', fg='white',height=2,width=5, command = lambda:self.startgame())
...
class Grid(Frame):
    def __init__(self, parent, r, c, n):
        ...
        for row in range(self.rows):
            ...
            for column in range(self.columns):
                self.blist[row][column] = Button(self,text = 'u',image = self.parent.emptyp, height=20, width=20, command=lambda row=row, column=column: self.clear(row, column))
                self.blist[row][column].bind('<Button-3>', lambda event, row = row, column = column: self.flag(row, column, self.nomines))
    ...
    def clear(self, xcord,ycord):
        ...
        if self.mines.mineslist[xcord][ycord] == 'M':        
            ...
            self.blist[xcord][ycord] = Label(self,height = 24, width = 24,bd = 1,relief = 'ridge',image = self.parent.minep)
    ...
    def flag(self, rr, cc, nn):
        ...
        if self.blist[rr][cc]['text'] == "u":
            ...
            self.blist[rr][cc] = Button(self,text = 'f',image = self.parent.flagp, height = 20, width = 20)
            self.blist[rr][cc].bind('<Button-3>', lambda event, rr = rr, cc = cc: self.flag(rr, cc, nn))
            self.blist[rr][cc].grid(row = rr, column = cc)
            self.minescoreup()
        # unflagging
        elif self.blist[rr][cc]['text'] == "f":
            self.blist[rr][cc].destroy()
            self.blist[rr][cc] = Button(self,text = 'u',image=self.parent.emptyp,height=20,width=20,command=lambda row=rr,column=cc:self.clear(row, column))
            self.blist[rr][cc].bind('<Button-3>', lambda event, rr = rr, cc = cc: self.flag(rr, cc, nn))
            self.blist[rr][cc].grid(row = rr, column = cc)
            self.minescoredown()
    ...
    def minescoreup(self):
        ...
        self.parent.minesleftl = Label(self.parent, text = self.parent.minesleft)
        self.parent.minesleftl.grid(row = 0, column = 3, sticky = 'N')
    ...
    def aftergame(self):
        ...
        self.parent.lone = Label(self.parent, text = "")
        ...
        self.parent.lthree = Label(self.parent, text = " ")
        self.parent.lthree.grid(row = 4, column = 0)

        self.parent.playagainb = Button(self.parent, text = "Play Again", command = lambda: self.playagain())
